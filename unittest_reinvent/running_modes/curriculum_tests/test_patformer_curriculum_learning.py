...
from running_modes.configurations import InceptionConfiguration, AutomatedCurriculumLearningInputConfiguration, \
...
from unittest_reinvent.fixtures.paths import MAIN_TEST_PATH, PRIOR_PATH, PATFORMER_PRIOR_PATH
from unittest_reinvent.fixtures.test_data import CELECOXIB


class TestPatformerCurriculumLearning(unittest.TestCase):
    ...
    def setUp(self):
        ...
        prior_config = ModelConfiguration(model_type.PATFORMER, model_regime.INFERENCE, PATFORMER_PRIOR_PATH)
        agent_config = ModelConfiguration(model_type.PATFORMER, model_regime.TRAINING, PATFORMER_PRIOR_PATH)
    ...
    def _create_configuration(self, smiles):
        ...
        curriculum_objectives = [CurriculumObjective(ScoringFunctionParameters(name=self.sf_enum.CUSTOM_PRODUCT,
                                                                               parameters=[curriculum_sf_parameters]), score_threshold=0.01)]

        curriculum_df = DiversityFilterParameters(self.filter_enum.NO_FILTER, 0.05, 25, 0.4)
        #TODO:
        curriculum_inception = InceptionConfiguration(memory_size=100, sample_size=10, smiles=[])
        learning_strategy = LearningStrategyConfiguration(parameters={"sigma": 120}, name=self.learning_strategy_enum.DAP_PATFORMER)

        curriculum_config = CurriculumStrategyInputConfiguration(name=self.cs_enum.PATFORMER,
                                                                 input=smiles,
        ...
        #TODO:
        production_inception = InceptionConfiguration(memory_size=100, sample_size=10, smiles=[])

        production_config = ProductionStrategyInputConfiguration(name=self.ps_enum.PATFORMER,
                                                                 input=smiles,
                                                                 scoring_function=
                                                            ScoringFunctionParameters(
