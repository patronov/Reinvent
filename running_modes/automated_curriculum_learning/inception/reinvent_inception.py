...
class ReinventInception(BaseInception):
    ...
    def _load_to_memory(self, scoring_function, prior):
        if len(self.configuration.smiles):
            standardized_and_nulls = [self._chemistry.mol_to_smiles(self._chemistry.smile_to_mol(smile)) for smile in smiles]
